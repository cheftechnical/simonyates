          ...
          <table className="border-gray-700 border-2 mt-6 w-full">
            <thead>
              <tr>
                ...
                <th className="bg-gray-700 border-gray-700 border-1 p-2 text-left text-gray-50 text-sm">
                  Host
                </th>
                <th className="bg-gray-700 border-gray-700 border-1 p-2 text-left text-gray-50 text-sm">
                  Title
                ...
                <th className="bg-gray-700 border-gray-700 border-1 p-2 text-center text-gray-50 text-sm">
                  Links
                </th>
              </tr>
            </thead>
            <tbody>
              {appearances.map((appearance, index) => (
                <tr key={index}>
                  ...
                  <td className="border-gray-100 border-1 p-2 text-sm">
                    {appearance.host}
                  </td>
                  <td className="border-gray-100 border-1 p-2 text-sm">
                    {appearance.title}
                  ...
                  <td className="border-gray-100 border-1 p-2 text-sm text-center">
                    {(appearance.links && appearance.links.length > 0) ||
                    (appearance.downloads &&
                      appearance.downloads.length > 0) ? (
                      <div className="flex flex-col gap-1 items-center">
                        {appearance.links &&
                          appearance.links.map((link, linkIndex) => (
                            <a
                              key={`link-${linkIndex}`}
                              href={link.url}
                              target="_blank"
                        ...
                        {appearance.downloads &&
                          appearance.downloads.map((download, downloadIndex) => (
                            <a
